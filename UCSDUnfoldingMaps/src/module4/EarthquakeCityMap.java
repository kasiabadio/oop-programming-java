...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		boolean flag = false;
		for (Marker cm: countryMarkers) {
			int quakeCounterLand = 0;	
			int quakeCounterOcean = 0;
			for (Marker em: quakeMarkers) {
				EarthquakeMarker m = (EarthquakeMarker) em;
				if (m.isOnLand()) {
					String country = (String) m.getProperty("country");
					if (country == cm.getProperty("name")) {
						quakeCounterLand += 1;
					}
				} else {
					quakeCounterOcean += 1;
			...
			if (flag == false) {
				System.out.println("OCEAN QUAKES: " + quakeCounterOcean);
				flag = true;
			}
			
			if (quakeCounterLand > 0) {
				System.out.println(cm.getProperty("name") + " " + quakeCounterLand);				
			}
